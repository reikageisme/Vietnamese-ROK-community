...
export function SearchResults({ initialQuery }: { initialQuery: string }) {
  ...
  const load = useCallback(async () => { if (submitted.trim().length < 2) return; const response = await fetch(`/api/forum/search?q=${encodeURIComponent(submitted)}&page=${page}`); const body = await response.json(); if (!response.ok) throw new Error(body.error); setData(body); }, [page, submitted]);
