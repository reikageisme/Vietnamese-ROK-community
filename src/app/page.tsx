...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <section className="stats"><div className="shell stats-grid"><div><strong>{kingdoms.length}</strong><span>Kingdom đang theo dõi</span></div><div><strong>{formatCompact(kingdoms.reduce((sum, item) => sum + item.power, 0))}</strong><span>Tổng lực chiến demo</span></div><div><strong>02</strong><span>Collector sẵn sàng</span></div><div><strong>94%</strong><span>Độ phủ trung bình</span></div></div></section>
      <div className="shell page-stack">
        <section><SectionHeading title="Trung tâm dữ liệu" href="/kingdoms" action="Mở dashboard" /><div className="home-data-grid"><Link className="home-feature-card kingdoms-feature" href="/kingdoms"><Badge>KINGDOM INTELLIGENCE</Badge><h2>Seed Ranking<br/>toàn cộng đồng</h2><p>So sánh lực chiến, KP, quân chết, T4/T5 và độ phủ của từng lượt quét.</p><span>Khám phá Kingdom →</span></Link><Link className="home-feature-card kvk-feature" href="/kvk"><Badge>KVK COMMAND CENTER</Badge><h2>Trại, phe và<br/>bản đồ chiến sự</h2><p>Nhóm kingdom theo camp, theo dõi tiến trình và sức mạnh tổng hợp.</p><span>Mở trung tâm KvK →</span></Link><Link className="home-feature-card scan-feature" href="/scans"><Badge>DATA PIPELINE</Badge><h2>Scan có ảnh<br/>và nguồn xác minh</h2><p>Từ điện thoại tới OCR, hàng chờ duyệt rồi mới xuất hiện trên web.</p><span>Xem các lượt quét →</span></Link></div></section>
