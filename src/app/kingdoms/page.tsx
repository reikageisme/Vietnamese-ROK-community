...
export default async function KingdomsPage() {
  ...
  return <div className="data-page">
    <section className="data-hero"><div className="shell data-hero-inner"><div><span className="data-eyebrow"><i /> ROKVIET INTELLIGENCE</span><h1>Bản đồ sức mạnh<br/><em>Kingdom Việt Nam</em></h1><p>Dữ liệu thật do agent điện thoại gửi về, có thời gian quét và mức cần kiểm tra rõ ràng. Không có số liệu giả cho Kingdom chưa được quét.</p></div><div className="hero-metrics"><div><strong>{catalog.length}</strong><span>Kingdom trong danh mục</span></div><div><strong>{compact(totalPower)}</strong><span>Tổng điểm đã quét</span></div><div><strong>{coverage}%</strong><span>Độ phủ trung bình</span></div><div><strong>{latest ? new Intl.DateTimeFormat("vi-VN", { dateStyle: "short" }).format(latest) : "—"}</strong><span>Cập nhật mới nhất</span></div></div></div></section>
    <div className="shell data-stack"><section className="data-panel"><div className="panel-heading"><div><span className="panel-kicker">SEED RANKING</span><h2>Xếp hạng Kingdom</h2><p>{scanned.length} Kingdom đã có dữ liệu; các Kingdom còn lại chỉ là danh mục nền và sẽ được scheduler xếp hàng khi có nhân vật truy cập.</p></div><span className="live-pill"><i /> LIVE DATABASE</span></div><KingdomExplorer kingdoms={rows} /></section></div>
  </div>;
}
