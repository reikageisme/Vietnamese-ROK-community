...
export async function consumeRegistrationAttempt(ip: string) {
  const count = await incrementWithWindow(`rokviet:auth:register:${keyPart(ip)}`, 60 * 60);
  return { allowed: count <= 5, remaining: Math.max(0, 5 - count) };
}

export async function isLoginLocked(email: string) {
  const client = await ensureRedis();
  return Number(await client.get(`rokviet:auth:login:${keyPart(email)}`)) >= 5;
}

export async function recordFailedLogin(email: string) {
  return incrementWithWindow(`rokviet:auth:login:${keyPart(email)}`, 15 * 60);
}

export async function clearFailedLogins(email: string) {
  const client = await ensureRedis();
  await client.del(`rokviet:auth:login:${keyPart(email)}`);
}
